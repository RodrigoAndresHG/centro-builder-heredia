    ...
    <div className="flex min-h-dvh bg-neutral-950 text-white">
      <aside className="hidden min-h-dvh w-72 border-r border-neutral-800 bg-neutral-950 px-5 py-6 lg:block">
        <Link href="/app" className="block">
          <p className="text-sm font-semibold text-white">Centro Builder</p>
          <p className="mt-1 text-xs font-medium text-teal-300">HeredIA Workspace</p>
        </Link>
        ...
        <nav className="mt-8 space-y-1" aria-label="Area privada">
          {appItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="block rounded-lg border border-transparent px-3 py-2.5 text-sm font-semibold text-neutral-300 transition hover:border-neutral-800 hover:bg-neutral-900 hover:text-white"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="mt-8 rounded-xl border border-teal-400/20 bg-teal-400/10 p-4">
